...
Feature: Admin User Management (CONSOLIDATED)
  ...
  #   - Company Target: E2ECOMPANYUID000000000000001 (e2e-company@example.test)
  #   - Influencer Target: E2EINFLUENCERUID000000000001 (norbertmarchewka - Instagram OAuth)
  #
  ...
  Scenario: Admin performs all user management operations (super consolidated)
    ...
    Given the target user "E2EINFLUENCERUID000000000001" is synced from Firestore
    ...
    # ----- BAN COMPANY USER -----
    Given the target user "E2ECOMPANYUID000000000000001" is synced and has status "ACTIVE" and role "COMPANY"
    When the admin bans user "E2ECOMPANYUID000000000000001" with reason "Terms of service violation"
    Then the response status should be 200
    ...
    # ----- BAN INFLUENCER USER -----
    Given the target user "E2EINFLUENCERUID000000000001" is synced and has status "ACTIVE" and role "INFLUENCER"
    When the admin bans user "E2EINFLUENCERUID000000000001" with reason "Spam activity detected"
    Then the response status should be 200
    And the user "E2EINFLUENCERUID000000000001" should have status "BANNED"

    # ----- UNBAN INFLUENCER USER -----
    When the admin unbans user "E2EINFLUENCERUID000000000001"
    Then the response status should be 200
    And the user "E2EINFLUENCERUID000000000001" should have status "ACTIVE"

    # ========== FINAL CLEANUP ==========
    # Ensure both users are in ACTIVE state after test
    When the admin sets user "E2ECOMPANYUID000000000000001" status to "ACTIVE"
    Then the response status should be 200
    When the admin sets user "E2EINFLUENCERUID000000000001" status to "ACTIVE"
    Then the response status should be 200
