...
Feature: Multi-User Session Isolation
  ...
  Scenario Outline: Company user logs in via multi-user infrastructure
    ...
    Examples:
      ...
      | FashionCo | E2E_COMPANY_001     | e2e.company@test.com | e2e-emulator-password  |
  ...
  Scenario Outline: Admin logs in with automatic 2FA completion
    ...
    Examples:
      | alias      | firebaseUid                      | email                         | password          |
      | AdminUser  | E2E_ADMIN_001     | e2e.admin@test.com | e2e-emulator-password  |
  ...
  Scenario: All user types logged in simultaneously
    ...
    Given "AdminUser" logs in as ADMIN with Firebase UID "E2E_ADMIN_001" email "e2e.admin@test.com" password "e2e-emulator-password" and completes 2FA
  ...
  Scenario: Company and influencer can interact with isolated sessions
    # Company logs in
    Given "BrandCo" logs in as COMPANY with Firebase UID "E2E_COMPANY_001" email "e2e.company@test.com" password "e2e-emulator-password"
    Then "BrandCo" should be authenticated

    # Influencer logs in via OAuth
    Given "InfluencerA" logs in as INFLUENCER via OAuth with Firebase UID "E2E_INFLUENCER_001"
