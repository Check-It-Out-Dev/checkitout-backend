# checkItOut — Business Domain Model
...
>
> **Audience.** Anyone evaluating or building on the platform — read this first to
...
> (It also served as the briefing document for the independent penetration tests.)
>
...
## 3. Trust boundaries & sensitive assets
...
These are the highest-value security artifacts: each edge is a legal
...
/ Snapchat are commented-out stubs that throw). Security-sensitive surface — the state/CSRF and
...
- **FINDING — storage webhook fail-open.** The HMAC check (`X-Firebase-Signature`) is **skipped
  ...
  `finalize`/`delete` events to mutate file-tracking/quota state. **Security-critical surface.** Even when
...
current public API surface. Flagged so readers don't expect `/subscription/**`.
