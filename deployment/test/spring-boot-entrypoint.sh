#!/bin/sh
# =============================================================================
# SPRING BOOT ENTRYPOINT - SIMPLIFIED VERSION
# Purpose: Wait for secrets file and start Spring Boot application
# Note: Init container strips TEST_ prefix, so secrets are unprefixed
...
# NOTE: Not checking POSTGRES_USER and POSTGRES_PASSWORD (only for PostgreSQL container)
MISSING_SECRETS=""

# Database secrets (for application connection)
# NOTE: DATABASE_NAME and DATABASE_USERNAME are hardcoded in application-test.yml
# Only DATABASE_PASSWORD is required from secrets
...
if [ -n "$MISSING_SECRETS" ]; then
    ...
    echo "These secrets should exist in GSM with TEST_ prefix"
fi

# Log configuration for debugging
echo "📋 Configuration:"
echo "   Spring Profile: ${SPRING_PROFILES_ACTIVE:-test}"
echo "   Database Host: ${DATABASE_HOST:-postgres}"
echo "   Database Port: ${DATABASE_PORT:-5432}"
echo "   Server Port: ${SERVER_PORT:-8082}"
echo "   Secrets Mount Path: ${SECRETS_MOUNT_PATH:-/app/config}"
echo "   Java Options: ${JAVA_OPTS}"

# Create log directory if it doesn't exist
LOG_DIR="/var/log/instagram-platform/test/application"
if [ ! -d "$LOG_DIR" ]; then
...
# Source the entire secrets file to make ALL variables available as environment variables
echo "📋 Loading all secrets as environment variables..."
...
# Debug: Verify critical environment variables are set (values masked - NEVER log actual values!)
echo "🔍 Verifying exported variables (existence only, no values)..."

# Database credentials (name/username hardcoded in application-test.yml, only password from secrets)
...
# Verify critical database password is set (name/username are in application-test.yml)
if [ -z "$DATABASE_PASSWORD" ]; then
    echo "❌ ERROR: Missing DATABASE_PASSWORD!"
    echo "Required from GSM: TEST_DATABASE_PASSWORD"
    exit 1
fi

# Export Spring datasource password (name/username come from application-test.yml)
export SPRING_DATASOURCE_PASSWORD="${DATABASE_PASSWORD}"
export SPRING_LIQUIBASE_PASSWORD="${DATABASE_PASSWORD}"

# If DATABASE_NAME/USERNAME are provided by secrets, also export them (override yml)
if [ -n "$DATABASE_NAME" ]; then
    export SPRING_DATASOURCE_URL="jdbc:postgresql://${DATABASE_HOST:-postgres}:${DATABASE_PORT:-5432}/${DATABASE_NAME}"
fi
if [ -n "$DATABASE_USERNAME" ]; then
    export SPRING_DATASOURCE_USERNAME="${DATABASE_USERNAME}"
    export SPRING_LIQUIBASE_USER="${DATABASE_USERNAME}"
fi
